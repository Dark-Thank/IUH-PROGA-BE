...
public class AiServiceImpl implements AiService {
    ...
    public void initRagKnowledgeBase() {
        try (InputStream is = getClass().getResourceAsStream("/rag/task_decomposition_samples.json")) {
            if (is != null) {
                ...
                log.info("Successfully loaded {} RAG Task Decomposition sample datasets into AI-Service memory.",
                        ragSamples.size());
            } else {
    ...
    public TaskDecompositionResponse decomposeRequirements(TaskDecompositionRequest request) {
        ...
        // Fetch existing message history in this specific thread BEFORE saving current
        // prompt
        ...
        String ragSourceRef = matchedRagSample != null
                ? (String) matchedRagSample.getOrDefault("evidenceBenchmark",
                        (String) matchedRagSample.getOrDefault("sourceReference", "PMBOK 7th Edition & IEEE Std 12207"))
                : "PMBOK 7th Edition & Scrum Guide Standards";
        String ragSourceUrl = allCitationUrls.isEmpty() ? "https://scrumguides.org/scrum-guide.html"
                : allCitationUrls.get(0);

        // Perform Vector Similarity Search over PgVector Store if available
        List<org.springframework.ai.document.Document> vectorDocs = vectorStoreService
                .searchSimilarDocuments(request.getRequirementText(), 3);
        ...
        try {
            ...
            String respSourceRef = (matchedRagSample != null)
                    ? (String) matchedRagSample.getOrDefault("evidenceBenchmark",
                            (String) matchedRagSample.getOrDefault("sourceReference", ragSourceRef))
                    : (String) parsed.getOrDefault("sourceReference", ragSourceRef);
            String respSourceUrl = (matchedRagSample != null)
                    ? (String) matchedRagSample.getOrDefault("legalTechnicalUrl",
                            (String) matchedRagSample.getOrDefault("sourceUrl", ragSourceUrl))
            ...
            if (!isDataSufficient) {
                ...
                List<Map<String, Object>> tasksRaw = (List<Map<String, Object>>) parsed.getOrDefault("tasks",
                        Collections.emptyList());
                taskItems = tasksRaw.stream().map(t -> TaskDecompositionResponse.DecomposedTaskItem.builder()
                        ...
                        .estimatedDays(
                                t.get("estimatedDays") != null ? ((Number) t.get("estimatedDays")).intValue() : 2)
                        .bufferDays(t.get("bufferDays") != null ? ((Number) t.get("bufferDays")).intValue() : 0)
                        .assignedRole((String) t.getOrDefault("assignedRole",
                                (String) t.getOrDefault("recommendedRole", "Backend Developer")))
                        .suggestedMemberName((String) t.get("suggestedMemberName"))
                        .riskWarning((String) t.get("riskWarning"))
                        .build()).collect(Collectors.toList());
            }

            List<TaskDecompositionResponse.RagCitationItem> citationItems = List.of(
                    TaskDecompositionResponse.RagCitationItem.builder()
                            ...
                            .snippet(
                                    "Quy định chuẩn hóa các giai đoạn vòng đời phát triển phần mềm (Software Lifecycle Processes) từ phân tích yêu cầu, kiến trúc DB đến kiểm thử.")
                    ...
                    TaskDecompositionResponse.RagCitationItem.builder()
                            ...
                            .snippet(
                                    "Quy chuẩn bóc tách Sprint 1-2 tuần, quản lý Product Backlog, phân công vai trò (PO/Dev/QA) và ước tính nỗ lực công việc.")
                            .priorityLevel("RẤT CAO")
                            .build(),
                    TaskDecompositionResponse.RagCitationItem.builder()
                            ...
                            .snippet(
                                    "Tiêu chuẩn kiểm soát rủi ro bảo mật web: Mã hóa dữ liệu AES-256, xác thực Token JWT stateless RFC 7519 và phân quyền RBAC.")
                            .priorityLevel("CAO")
                            .build(),
                    TaskDecompositionResponse.RagCitationItem.builder()
                            ...
                            .snippet(
                                    "Mẫu ước tính thời gian (Estimate Benchmark 1-3 ngày/task), thời gian dự phòng rủi ro (bufferDays) và phân bổ nhân lực vừa sức.")
                            .priorityLevel("CAO")
                            .build());
            ...
            if (matchedRagSample != null && matchedRagSample.containsKey("tasks")) {
                ...
                for (Map<String, Object> st : sampleTasks) {
                    fallbackItems.add(TaskDecompositionResponse.DecomposedTaskItem.builder()
                            ...
                            .estimatedDays(
                                    st.get("estimatedDays") != null ? ((Number) st.get("estimatedDays")).intValue() : 2)
            ...
            List<TaskDecompositionResponse.RagCitationItem> fallbackCitations = List.of(
                    TaskDecompositionResponse.RagCitationItem.builder()
                            .anchorCategory("Tiêu chuẩn Quốc tế IEEE 12207")
                            .title("IEEE Std 12207-2017: Systems and Software Lifecycle Processes")
                            .sourceUrl("https://standards.ieee.org/ieee/12207/6182/")
                            .snippet("Quy định chuẩn hóa các giai đoạn vòng đời phát triển phần mềm.")
                            .priorityLevel("RẤT CAO")
                            .build(),
                    TaskDecompositionResponse.RagCitationItem.builder()
                            .anchorCategory("Quy chuẩn Agile Scrum Guide 2020")
                            .title("The Scrum Guide (2020 edition)")
                            .sourceUrl("https://scrumguides.org/scrum-guide.html")
                            .snippet("Quy chuẩn bóc tách Sprint 1-2 tuần và quản lý Product Backlog.")
                            .priorityLevel("RẤT CAO")
                            .build());
    ...
    public AiChatMessageResponse analyzePmProgressAndRisk(long spaceId) {
        ...
        String userPrompt = String.format("""
                ...
                tasks.stream()
                        .map(t -> String.format("- [%s] %s (Trạng thái: %s, Ưu tiên: %s)", t.getId(), t.getTitle(),
    ...
    private String generateMockAiResponse(String systemPrompt, String userPrompt) {
        if (systemPrompt.contains("Requirement Agent")) {
            if (systemPrompt.contains("LƯỢT ĐÀM THOẠI ĐẦU TIÊN")) {
                ...
                if (promptLower.contains("nhà hàng") || promptLower.contains("quản lý bàn")
                        || promptLower.contains("thực đơn") || promptLower.contains("gọi món")) {
                ...
                } else if (promptLower.contains("y tế") || promptLower.contains("telehealth")
                        || promptLower.contains("bác sĩ") || promptLower.contains("bệnh nhân")
                        || promptLower.contains("khám")) {
                ...
                } else if (promptLower.contains("đồ án") || promptLower.contains("khóa luận")
                        || promptLower.contains("iuh")) {
            ...
            if (promptLower.contains("y tế") || promptLower.contains("telehealth") || promptLower.contains("bác sĩ")
                    || promptLower.contains("bệnh nhân") || promptLower.contains("khám")) {
                return """
                        {
    ...
    private String buildSystemPrompt(AgentType agentType) {
        return switch (agentType) {
            case REQUIREMENT ->
                "Bạn là Requirement Agent (Business Analyst / PO) của PROGA. Hỗ trợ phân rã bài toán và làm rõ yêu cầu nghiệp vụ.";
            case PM ->
                "Bạn là PM Agent của PROGA. Hỗ trợ tóm tắt tiến độ, dự báo rủi ro deadline và tư vấn quản lý dự án.";
